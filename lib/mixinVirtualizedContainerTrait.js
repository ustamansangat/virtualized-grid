...
module.exports = function (self, options) {
  ...
  if (!_.isFunction(self.rowEl)) {
    throw new Error('Method rowEl() is not implemented');
  }
  ...
  self.listenTo(self.collection, 'change', function (model, collection) {
    //TODO
    self.$('.item-row').toArray().map(function (row) {
      return $(row).data('index');
    });
  });
  self.listenTo(self.collection, 'add remove reset change', function () {
  });
  self.listenTo(self.collection, 'add remove reset change', function () {
  });
  self.listenTo(self.collection, 'add remove reset change', function () {
...
//    }
  ...
  function getInstructions($rows, prefix, scrollTop, marks) {
      ...
      if ($rows.size() === 0 || scrollTop >= _.last(marks) || scrollTop + self.$el.height() <= prefix) {
...
//        console.log('Rendered', instructions.from, 'to', index - instructions.direction);
  ...
  function render () {
    ...
    if (self.$('.item-row:first').data('index') === instructions.from
